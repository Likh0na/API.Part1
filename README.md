# API.Part1
RaceDay is a race/event management system. Organisers create events with one or more categories, participants register for a category, and results are captured once the race is complete.
...
Adding a new role later only requires a new row in Roles, not a schema change.

EventRegistrations has a status column with a CHECK constraint. Not shown in the original brief's example table, but added so an enrolment can be tracked through Pending to Confirmed to Cancelled, which the API endpoint plan (enrolment/cancellation endpoints) depends on.
...
The SQL script in RaceDay_Schema.sql matches RaceDay_ERD.pdf exactly — same tables, same columns, same primary/foreign keys, same cardinality. There are no unexplained differences between the two.
